...
def add_installation_command_to_cluster(cluster, account_id):
    install_command = account.get_agent_installation_command_by_id(account_id)
    if install_command is None:
        cluster["notes"].append(
            f"Failed to get agent installation command for account: {account_id}"
        )
        cluster['status'] = 'Failed to get agent installation command'
        return cluster
...
def setup_cluster(cluster):
    ...
    if not sedai_account_id:
        ...
        if not os.path.exists(ret_cluster['service_account_file_path']):
            ret_cluster["notes"].append(
                f"The service account file {ret_cluster['service_account_file_path']} does not exist"
            )
            return ret_cluster

    add_installation_command_to_cluster(ret_cluster, sedai_account_id)
    return ret_cluster
